...
from re_agent import RTCM_agent
...
if __name__ == '__main__':
    ...
    # 配置日志输出
    log_file = "re_log/RTCM_episodes_{}_malicious_{}_device_{}_task_{}.txt".format(args.epochs, args.malicious, args.device_num, args.task_arrival_prob)
    ...
    for epoch in range(args.epochs):
        ...
        for t in range(args.T):
            ...
            # 生成信誉矩阵 (数据驱动部分)
            with torch.no_grad():
                r_data = agent.re_net.forward(dynamic_re, static_re, neighbor_matrix)
        ...
        if len(agent.replay_buffer) >= args.batch_size:
            ...
            if (epoch + 1) % 5 == 0 or epoch == args.epochs - 1:
                save_path = f"re_model/RTCM_model_device_{args.device_num}_task_{args.task_arrival_prob}_malicious_{args.malicious}.pt"
