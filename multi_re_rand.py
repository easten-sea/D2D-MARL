    ...
    for epoch in range(args.epochs):
        ...
        for t in range(args.T):
            ...
            for server_id in range(server_manager.num_servers):
                ...
                task_em, dynamic_re, static_re, neighbor_matrix, ex_r_d, ex_C_i = current_state  # 解包状态，包括ex_r_d和ex_C_i
                ...
                for i in range(len(actions[0])):
                    ...
                    # 如果决策是向相邻服务器卸载(index = args.neighbors_num)
                        ...
                        # 确定卸载到哪个相邻服务器
                        server_choice = random.randint(0, len(server['neighbors']) - 1)
                        ...
                        # 计算能耗与信誉相关
                        energy_cost = (args.neighbors_num + 1) * (1 - target_server_reputation)
                    ...
                    # 否则，向本服务器管理的邻居设备卸载
                        ...
                        # 计算能耗 - 与邻居距离和信誉相关
                        energy_cost = (neighbor_index + 1) * (1 - r_d[neighbor_device_local_id])
                        e_reward -= energy_cost
                        # 获取邻居设备的全局ID
                ...
                for i in range(args.device_num):
                    ...
                    for j in range(args.neighbors_num):
                        ...
                        global_neighbor_id = server_manager.get_global_device_id(
                            server_id, next_neighbor_matrix[i][j])
                        neighbor_device = server_manager.get_device_info(global_neighbor_id)
                        co += neighbor_device[1] + neighbor_device[2]
                    ...
                    # 结合自身安全能力和邻居安全能力计算静态信誉
                    next_static_re[i] = args.se_ca * (device[1] + device[2]) + (
                            1 - args.se_ca) * co / args.neighbors_num
            ...
            if len(agent.replay_buffer) >= args.batch_size:
                ...
                for _ in range(train_iterations):
                    ...
                    for i in range(len(state_batch)):
                        ...
                        task_em, dynamic_re, static_re, neighbor_matrix, stored_ex_r_d, stored_ex_C_i = state_batch[i]
                        next_task_em, next_dynamic_re, next_static_re, next_neighbor_matrix, next_stored_ex_r_d, next_stored_ex_C_i = \
                        next_state_batch[i]
                        ...
                        with torch.no_grad():
                            ...
                            # 使用存储的下一状态的邻居服务器信誉向量
                            _, next_reputation_aware, _, _ = agent.re_aware(next_reputation_matrix, next_r_d,
                                                                            next_stored_ex_r_d)
                    ...
                    # 转换为张量 - 确保一致的形状
                    critic_values_tensor = torch.stack(critic_values).squeeze(-1)  # 确保形状是 [batch_size]
                    next_values_tensor = torch.stack(next_values).squeeze(-1)  # 确保形状是 [batch_size]
                    ...
                    reputation_loss = 0
                    for i in range(len(state_batch)):
                        task_em, dynamic_re, static_re, neighbor_matrix, _, _ = state_batch[i]
                    ...
                    for i in range(len(state_batch)):
                        task_em, dynamic_re, static_re, neighbor_matrix, stored_ex_r_d, _ = state_batch[i]
                        ...
                        # 获取Actor的当前策略动作
                        current_actions = agent.actor(reputation_matrix, reputation_aware, task_em, neighbor_matrix,
                                                      r_mask)

                        # 获取当前策略的Q值
                        agent.critic.eval()
                        q_value = agent.critic(dynamic_re, static_re, task_em, neighbor_matrix, r_mask,
                                               reputation_aware, current_actions)
...
if __name__ == '__main__':
    ...
    # 配置日志输出
    log_file = "re_log/multi_random_server_episodes_{}_servers_{}_malicious_{}_device_{}_task_{}.txt".format(
