...
class D3QNNetwork(nn.Module):
    def __init__(self, args):
        ...
        self.advantage_stream = nn.Sequential(
            nn.Linear(64, 32),
            nn.ReLU(),
            nn.Linear(32, args.device_num * (args.neighbors_num + 1))
        )
    ...
    def forward(self, dynamic_re, static_re, task_em, neighbor_matrix):
        ...
        # 应用嵌入层
        dynamic_emb = self.relu(self.dynamic_re_embedding(dynamic_re))
        static_emb = self.relu(self.static_re_embedding(static_re))
        task_emb = self.relu(self.task_embedding(task_em))
        neighbor_emb = self.relu(self.neighbor_embedding(neighbor_matrix))

        # 合并特征
        combined = torch.cat([dynamic_emb, static_emb, task_emb, neighbor_emb], dim=0)
        combined = combined + torch.randn_like(combined) * 0.2
        features = self.feature_layer(combined)

        # 价值流和优势流
        value = self.value_stream(features)
        advantages = self.advantage_stream(features)
        advantages_shaped = advantages.view(self.args.device_num, self.args.neighbors_num + 1)

        # 将value扩展到正确的形状 - 每个设备一个值
        value_expanded = value.expand(self.args.device_num)
        value_reshaped = value_expanded.unsqueeze(1)
        q_values = value_reshaped + advantages_shaped
...
class D3QNAgent:
    def __init__(self, args):
        ...
        # 创建策略网络和目标网络
        self.policy_net = D3QNNetwork(args).to(self.device)
        self.target_net = D3QNNetwork(args).to(self.device)
    ...
    def update(self, batch_size):
        ...
        for i in range(len(state_batch)):
            ...
            with torch.no_grad():
                ...
                target = reward_tensor[i] + (1 - done_tensor[i]) * (self.args.gamma * 0.5) * next_q_max
        ...
        loss = F.mse_loss(current_q_batch, target_q_batch)
        ...
        if self.update_counter % (self.args.target_update_freq * 3) == 0:  # 从5增至15
            self.target_net.load_state_dict(self.policy_net.state_dict())
...
if __name__ == '__main__':
    ...
    # 配置日志输出
    log_file = "re_log/test_d3qn_1.txt"
    logging.basicConfig(
        level=logging.INFO,
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % 5 == 0 or epoch == args.epochs - 1:
            save_path = f"re_model/d3qn_model_epoch_{epoch}.pt"
